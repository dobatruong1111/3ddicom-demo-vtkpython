...
class ScissorsPipeline:
    def __init__(self) -> None:
        ...
        self.polyDataMapper.SetInputData(self.polyData)
        # Actor
        property = vtk.vtkProperty()
        property.SetColor(colors.GetColor3d("Tomato"))
        self.polyDataActor = vtk.vtkActor()
        self.polyDataActor.SetMapper(self.polyDataMapper)
        self.polyDataActor.SetProperty(property)
        self.polyDataActor.VisibilityOff()
...
class CropFreehandInteractorStyle(vtk.vtkInteractorStyleTrackballCamera):
    ...
    def updateBrushModel(self):
        ...
        closedSurfacePolyData = self.pipeline.polyData
        ...
        self.pipeline.polyDataActor.VisibilityOn()
        self.GetInteractor().Render()
...
def main() -> None:
    ...
    render.AddActor(pipeline.polyDataActor)
    ...
    style = BeforeCropFreehandInteractorStyle(pipeline, imageData, map)
